...
function BookDescription() {
    ...
    const handleEditComment = async (e) => {
        ...
        try {
            await fetch(url_api + editComment.elementId, {
                method: "PUT",
                headers: {
                    Authorization: `Bearer ${token}`,
                    "Content-Type": "application/json",
                },
                body: JSON.stringify(editCommentInput), // Utilizza editCommentInput invece di editComment
            });
    ...
    return (
        <>
            ...
            <Modal show={showEditModal} onHide={() => setShowEditModal(false)}>
                ...
                <Modal.Body>
                    {/* Aggiungi un modulo simile a quello di creazione del commento per la modifica */}
                    <Form onSubmit={handleEditComment}>
                        {/* ... Altri campi di input per la modifica */}
                        <Row className="flex-column">
                            <Form.Group md="4" controlId="rate" style={{ width: "130px" }}>
                                <Form.Label><span className='fw-bold'>Voto:</span></Form.Label>
                                <Form.Control
